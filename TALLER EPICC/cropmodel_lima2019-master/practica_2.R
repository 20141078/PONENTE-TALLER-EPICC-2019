...
season_data %>% 
    ...
    geom_histogram(bins = 10, color="blue") + facet_wrap(var ~., scales = "free") + 
    theme_classic()
###
...
length(datos_clima$tmin)
length(daily_data$Yield)
#cbind(datos_clima$date, datos_clima$tmin, season_data$Day1 season_data$Yield )
plot(datos_clima$date, datos_clima$tmin)
